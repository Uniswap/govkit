...
library L1CrossDomainMessengerDecoder {
    ...
    function decode(Call memory l1CrossDomainMessengerCall) internal pure returns (
        address,
        address,
        uint32,
        Call memory
    ) {
        require(l1CrossDomainMessengerCall.data.getSelector() == IL1CrossDomainMessenger.sendMessage.selector, SelectorMismatch());
        ...
        require(crossChainAccountData.getSelector() == ICrossChainAccount.forward.selector, SelectorMismatch());

        (address remoteTarget, bytes memory remoteData) = abi.decode(
            crossChainAccountData.stripSelector(), (address, bytes)
        );
