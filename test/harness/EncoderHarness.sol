...
contract EncoderHarness {
    function encodeFxRoot(address fxRoot, address fxReceiver, Call[] memory remoteCalls)
        external
        pure
        returns (Call memory)
    {
    ...
    function encodeInbox(
        ...
        return InboxEncoder.encode(
            inbox, timelock, gasLimit, maxFeePerGas, maxSubmissionCost, remoteCall
        );
    }
    ...
    function encodeL1CrossDomainMessenger(
        ...
        return L1CrossDomainMessengerEncoder.encode(
            l1CrossDomainMessenger, crossChainAccount, gasLimit, remoteCall
        );
    }

    function encodeOptimismPortal2(address portal, uint64 gasLimit, Call memory remoteCall)
        external
        pure
        returns (Call memory)
    {
