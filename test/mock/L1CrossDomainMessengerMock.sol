...
contract L1CrossDomainMessengerMock {
    function sendMessage(address target, bytes calldata message, uint32 minGasLimit) external payable returns (
        address,
        address,
        bytes memory,
        uint32
        ...
        (address forwardTarget, bytes memory forwardData) = abi.decode(message[4:], (address, bytes));
